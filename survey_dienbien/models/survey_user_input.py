...
class SurveyUserInput(models.Model):
    ...
    diem_cao_nhat = fields.Integer('Điểm cao nhất', readonly=True)
    so_lan_lam = fields.Integer(string='Tổng số lượt thi', readonly=True, compute='_compute_so_lan_lam', store=True)
    xep_loai = fields.Char(string='Xếp loại', required=False, readonly=True,
    ...
    def create(self, vals_list):
        ...
        for record in records:
            if record.partner_id and record.survey_id:
                # Tìm tất cả các bản ghi có cùng survey_id và partner_id, bao gồm cả bản ghi hiện tại
                user_inputs = self.env['survey.user_input'].search([
                    ('survey_id', '=', record.survey_id.id),
                    ('partner_id', '=', record.partner_id.id)
                ])
                # Kiểm tra loại tính điểm và tìm điểm cao nhất
                ...
                # Cập nhật diem_cao_nhat cho tất cả các bản ghi tìm thấy
                user_inputs.write({'diem_cao_nhat': diem_cao_nhat})
            else:
                # Nếu không có partner_id hoặc survey_id, đặt diem_cao_nhat là 0
