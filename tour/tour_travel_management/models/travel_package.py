...
class TravelPackage(models.Model):
    ...
    @api.depends("arrival_date", "return_date", "fixed_tour_days")
    def _compute_package_days(self):
        ...
        for package in self:
            ...
            elif package.fixed_tour_days:
                nights = package.fixed_tour_days
            if nights:
                package.package_total_days = "{} Nights / {} Days".format(
                    str(nights), str(nights + 1)
                )
                package.days = nights + 1
                package.nights = nights
    
    def write(self,vals):
        if vals.get('package_type') and vals.get('package_type') == 'domestic':
            vals['country_id'] = self.env.user.company_id.country_id.id
        else:
            vals['country_id'] = False   
            
        return super(TravelPackage,self).write(vals)

    def _compute_total_registration(self):
        sale_order_obj = self.env["sale.order"]
        for package in self:
            confirm_reg = sale_order_obj.search_count(
    ...
    def _compute_cost_per_person(self):
        self.update({"cost_per_person": 0.0, "cost_per_child": 0.0})

    def _compute_sell_per_person(self):
        self.update({"sell_per_person": 0.0, "sell_per_child": 0.0})
    ...
    total_passenger = fields.Integer(compute="_compute_total_passenger")
    total_passenger_all = fields.Integer(compute="_compute_total_passenger")
    total_registration = fields.Integer(compute="_compute_total_registration")
    company_id = fields.Many2one(
        "res.company", "Company", default=lambda self: self.env.company
    )
    cost_per_person = fields.Float(compute="_compute_cost_per_person", store=True)
    cost_per_child = fields.Float(compute="_compute_cost_per_person", store=True)
    sell_per_person = fields.Float(compute="_compute_sell_per_person", store=True)
    sell_per_child = fields.Float(compute="_compute_sell_per_person", store=True)
    ...
    fixed_tour_days = fields.Integer("Tour Days")
    package_create_date = fields.Date("Package Creation Date", copy=False)

    def name_get(self):
        result = []
        for template in self:
            name = "{} - {}".format(template.name, template.package_total_days)
            result.append((template.id, name))
        return result

    @api.onchange("type_of_package")
    def _onchange_type_of_package(self):
        self.fixed_tour_days = 0.0
        self.arrival_date = self.return_date = False

    @api.onchange("package_type")
    def _onchange_package_type(self):
        self.country_id = False
        if self.package_type == "domestic":
            self.write({'country_id': self.env.user.company_id.country_id.id})
    ...
    def create_itinerary_date(self, start_date, end_date):
        ...
        while start_date <= end_date:
            i = i + 1
            vals = {"itinerary_date": start_date,"name":"Day : " + str(i), "description": "Day" + str(i)}
            date_data_list.append((0, 0, vals))
            start_date = start_date + relativedelta(days=+1)
        return date_data_list

    def create_itinerary_by_days(self, fixed_tour_days):
        date_data_list = []
        i = 0
        while i < fixed_tour_days:
            i = i + 1
            day = self.env["day.selection"].search([("code", "=", i)])
            vals = {"days": day.id,"name":"Day : " + str(i), "description": "Day" + str(i)}
            date_data_list.append((0, 0, vals))
        return date_data_list

    def action_generate_itinerary_plan(self):
        if (
            self.type_of_package == "specific"
        ...
        elif self.type_of_package == "nonspecific" and self.fixed_tour_days:
            self.itinerary_ids = [(5,)]
            self.itinerary_ids = self.create_itinerary_by_days(self.fixed_tour_days)

    def action_fetch_cost_sale_price(self):
        for rec in self.group_costing_ids:
            rec.cost_price = round((
    ...
    def check_itinerary_ids(self):
        for rec in self:
            if not rec.itinerary_ids and rec.state == "confirm":
                raise ValidationError(
                    _("Please add itinerary lines before confirm the package '%s'.")
                    % (rec.name,)
                )
            if not rec.group_costing_ids and rec.state == "confirm":
                raise ValidationError(
                    _("Please add Group Costing before confirm the package '%s'")
                    % (rec.name,)
                )
    ...
    def _compute_total_passenger(self):
        ...
        for package in self:
            ...
            all_count = sum(len(ord.passenger_ids) for ord in all_orders)
            count = sum(len(order.passenger_ids) for order in orders)
            package.total_passenger = count
            package.total_passenger_all = all_count
            ...
            action.update(
                {
                    ...
                    "views": [
                        (
                            self.env.ref(
                                "tour_travel_management.passenger_list_tree"
                            ).id,
                            "tree",
                        ),
                        (
            ...
            action.update(
                {
                    ...
                    "views": [
                        (
                            self.env.ref("sale.view_quotation_tree_with_onboarding").id,
                            "tree",
                        ),
                        (
                            self.env.ref(
